...
namespace video
{
	...
	public:
		...
		IRenderTargetAttachment* getDepthStencil() const
		{
			return DepthStencil;
		}
		...
		//! Set multiple textures and depth/stencil texture or renderbuffer.
 		/** Set multiple textures for the render target.
		\param texture Array of texture objects. These textures are used for a color outputs.
		\param depthStencil Depth or packed depth-stencil attachment.
		or depth-stencil buffer. */
        virtual void setAttachments(const core::array<ITexture*>& texture, IRenderTargetAttachment* depthStencil)
        {
            os::Printer::log("Render Buffers unimplemented. Use Render Textures.", ELL_ERROR);
        }
        ...
        //! Set one attachment each for color and depth
        virtual void setAttachment(ITexture* texture, IRenderTargetAttachment* depthStencil)
        {
            core::array<ITexture*> textureArray(1);
            textureArray.push_back(texture);
            
            setAttachments(textureArray, depthStencil);
        }
	...
	protected:
		...
		IRenderTargetAttachment* DepthStencil;
